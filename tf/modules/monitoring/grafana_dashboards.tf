...
resource "grafana_dashboard" "node_info" {
  ...
  config_json = templatefile(
    "${path.module}/grafana_dashboards/node_info.json.tftpl",
    {
      ...
      zfs_pool_capacity     = grafana_library_panel.zfs_pool_capacity.uid
      zfs_pool_storage_used = grafana_library_panel.zfs_pool_storage_used.uid
      zfs_pool_health       = grafana_library_panel.zfs_pool_health.uid
      unhealthy_disks       = grafana_library_panel.disks_unhealthy.uid
    }
  )
}
